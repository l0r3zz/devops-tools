...
ENDPOINTS = {
    0: {
        'version': 'version',
        'current': 'collector/role/current',
        'current-env': 'collector/env/current',
        'list': 'templates/list',
        'template-generic': 'templates/get/"generic"',
        },
    1: {
        'version': 'version',
        'current': 'collector/role/current',
...
ERROR_STRINGS = {
    'timeout': 'Connection  timed out on',
    'refused': 'Could not reach vigilanted on',
}
...
class BaseAPI(object):
    ...
    reused across different versions of the vigilante API. If querying for a
    ...
    :param host: (optional) Hostname or IP of vigilanteD.
    :type host: :obj:`string`
    :param port: (optional) Port on which to talk to vigilanteD.
    :type port: :obj:`int`
    :param ssl_verify: (optional) Verify vigilanteD server certificate.
...
`
    ...
    def _url(self, endpoint, path=None):
        ...
        :param endpoint: The VigilanteD API endpoint we want to query.
    ...
    def _query(self, endpoint, path=None, query=None,
        ...
        """This method actually querries VigilanteD. Provided an endpoint and an
        optional path and/or query it will fire a request at VigilanteD. If
        VigilanteD can be reached and answers within the timeout we'll decode
        the response and give it back or raise for the HTTP Status Code
        VigilanteD gave back.
        ...
        :param limit: (optional) Tell VigilanteD to limit it's response to this\
                number of objects.
        :type limit: :obj:`int`
        :param offset: (optional) Tell VigilanteD to start it's response from\
        ...
        :returns: The decoded response from VigilanteD
